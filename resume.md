# 简历项目描述（可直接使用 / 按需裁剪）

## 项目名称
TikTok Shop 爆品监测与选品分析系统

## 一句话简介
自研 TikTok Shop 美区商品数据采集与选品决策系统，自动完成「采集 → 存储 → 毛利测算 → 选品评分 → 可视化看板」全流程。

## 技术栈
Python · Playwright · 可插拔数据源架构 · EchoTik 开放 API · SQLite · 定时调度 + Windows 任务计划 · SVG 看板 · Git

## 项目亮点（面试讲这几点）
- 可插拔数据源：抽象 DataSource 接口，统一 demo（模拟）/ scraper（自建 Playwright 采集）/ api（第三方数据平台，如 Kalodata、EchoTik、FastMoss）三种来源，切换数据供给不碰业务管道。
- 第三方 API 落地：真实对接 EchoTik 开放接口（Basic 认证、GET 搜索接口、响应字段归一化），免费额度下跑通真实商品采集并落库（2026-08-13 实采：48 商品 / 20 达人 / 15 关键词，密钥文件自动加载）。
- 成熟采集能力：类目树浏览/搜索（l1~l3）、按类目分页采集与筛选（销量/价格/佣金/7天趋势排序）、多关键词批量去重、商品详情补全、日/周/月榜采集；落库近7天/30天销量、GMV、带货达人/视频数等经营字段并接入选品评分。
- 趋势与爆品预测：基于销量快照时间序列计算近7天/30天增量与增速、新品检测（14天起量）、爆品指数（速度×动能×新品），输出爆品预测榜与趋势面板。
- 店铺监控：卖家维度聚合（商品数/累计销量/GMV/均价）+ 关注店铺池 + 新上架商品检测与告警，实现竞品店铺跟卖跟踪。
- 类目洞察与蓝海指数：聚合类目规模/近7天销量/增速/CR4/CR10 集中度，用增速×分散度×进入门槛×市场量级计算蓝海指数并标记机会类目。
- 历史对比：每日产出新上架/价格异动/销量激增三类变化汇总，形成「昨日 vs 今日」状态差。
- 多源故障切换：抽象 FailoverSource 按 EchoTik→FastMoss→demo 顺序自动降级，结合请求级重试退避，实现无人值守稳定采集。
- 内容带货维度：直播带货榜（GMV/销量/峰值观看/时长）与短视频热度榜，覆盖 TikTok 直播/短视频双内容场景。
- 数据管道：采集、增量更新、价格/销量历史快照、断点续爬，形成每日自动更新的商品库。
- 定时调度：内置每日调度器 + Windows 任务计划一键注册，实现无人值守的日报产出与日志留存。
- 反爬实战：研究 TikTok X-Bogus/MsToken 签名与浏览器指纹风控，采用真实浏览器 + 页面内嵌 JSON 解析方案，兼容页面改版。
- 业务建模：内置跨境小包直发毛利模型（采购成本/平台佣金/物流/广告），输出预估毛利与毛利率。
- 选品评分：综合需求度（销量+视频播放）、竞争度（评论稀疏度）、利润分三维打分，输出 Top 爆品榜。
- 数据可视化：生成自包含 HTML 看板（类目销量、选品评分、销量趋势），离线可打开。
- 合规设计：仅采集公开数据、控制抓取频率、遵守平台规则；生产环境优先走第三方数据 API / 官方 Partner API。

## STAR 写法（示例）
- 背景：跨境电商选品依赖人工浏览，效率低且缺少量化依据。
- 任务：搭建一套自动化的 TikTok Shop 竞品/选品数据系统。
- 行动：设计可插拔数据源架构，实现 Playwright 采集器与第三方数据 API 适配器；设计 SQLite 分层存储与历史快照；构建毛利测算与三维选品评分模型；产出 HTML 分析看板与 CSV 导出。
- 结果：支持每日定时增量采集（免费额度下单次可入库约 50 商品），输出 Top 20 爆品榜与机会类目，将选品决策周期从数天缩短到分钟级；已通过 EchoTik 真实接口完成采集验证（关键词 + 类目 48 商品入库）。

## 面试可能被追问的点
- 为什么用 Playwright 而不是 requests？→ TikTok 需要执行 JS、有签名和滑块风控，真实浏览器绕过签名成本最低。
- 为什么做数据源抽象？→ 自建采集受反爬、账号、网络（代理）约束，生产环境通常购买第三方数据服务；抽象后数据来源可替换、可扩展，分析层完全无感。
- 怎么保证数据不重复？→ 商品表以 product_id 为主键，快照表记录每次变化，天然去重。
- 增量采集怎么做？→ 已存在商品只更新字段，同时追加价格/销量快照用于趋势。
- 如果页面改版了怎么办？→ 优先解析 __UNIVERSAL_DATA_FOR_REHYDRATION__ 内嵌 JSON；选择器失效时更新候选选择器列表。
- 为什么接 EchoTik API？→ 自建采集受反爬、账号、网络（代理）约束，第三方数据服务稳定且合规；不同平台鉴权/字段差异大，用统一 ProviderConfig + normalize_item 归一化。
- 法律合规？→ 仅采集公开页面、控制频率、遵守 robots.txt；生产环境优先走官方 Partner API 或第三方数据服务。
