...
def probe(page, url, label, shot):
    try:
        ...
        print(f"--- {label} ERROR:", str(exc)[:300])
...
def main() -> None:
    ...
    ap.add_argument("--category", default="603084")
    args = ap.parse_args()
    ...
    with sync_playwright() as p:
        ...
        for region in regions:
            ...
            page = ctx.new_page()
            probe(page, f"https://shop.tiktok.com/{region.lower()}/s?q={q}",
                  f"REGION {region} SEARCH", f"diag_{region}.png")
            probe(page, f"https://shop.tiktok.com/{region.lower()}/c/{args.category}",
                  f"REGION {region} CATEGORY /c/{args.category}", f"diag_{region}_cat.png")
            probe(page, f"https://shop.tiktok.com/{region.lower()}/s?cat={args.category}&q=",
                  f"REGION {region} SEARCH+CATFILTER", f"diag_{region}_catsearch.png")
            ctx.close()
        b.close()
